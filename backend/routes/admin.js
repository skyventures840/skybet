...
router.put('/matches/:id', adminAuth, async (req, res) => {
  try {
    ...
    // Trigger settlement if match is finished
    if (match.status === 'finished') {
      console.log(`[ADMIN] Triggering settlement for match ${match._id}`)
      betSettlementService.processSettlements().catch(err => console.error('Auto-settlement trigger error:', err))
    }
...
router.put('/matches/:matchId/result', adminAuth, [
  ...
  try {
    ...
    const sportTitle = match.leagueId?.name || match.sport || 'Unknown League'
    const commenceTime = match.startTime || new Date()
...
router.put('/odds/:eventId/result', adminAuth, [
  ...
  try {
    ...
    const sportKey = odds.sport_key || 'football'
    const sportTitle = odds.sport_title || 'Unknown League'
