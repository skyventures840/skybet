...
class OddsApiService {
  ...
  async _storeScoresAsync (scores) {
    try {
        ...
        updateOne: {
          ...
          update: {
            $set: {
              ...
              game_time: {
                period: score.game_time?.period,
                clock: score.game_time?.clock,
                display_clock: score.game_time?.display_clock
              },
      ...
      if (bulkOps.length > 0) {
        ...
        // Sync to Match collection
        const matchOps = scores.map(score => {
          const updateObj = {
          ...
          if (score.scores && Array.isArray(score.scores)) {
            ...
            if (homeScoreObj) updateObj.homeScore = parseInt(homeScoreObj.score)
            if (awayScoreObj) updateObj.awayScore = parseInt(awayScoreObj.score)
          }

          // Parse minute from clock if available (e.g., "45:00")
          if (score.game_time?.clock) {
            const parts = score.game_time.clock.split(':')
            if (parts.length >= 1) {
              const minute = parseInt(parts[0])
              if (!isNaN(minute)) {
                updateObj['liveData.minute'] = minute
              }
            }
          }
        ...
        if (matchOps.length > 0) {
          try {
            await Match.bulkWrite(matchOps, { ordered: false })
            console.log(`Synced ${matchOps.length} scores to Match collection`)
          } catch (matchError) {
            console.error('Error syncing scores to Match collection:', matchError.message)
          }
        }
      }
    } catch (error) {
  ...
  async _mergeAdditionalMarkets (eventData, targetMarkets) {
    try {
      ...
      // const existingBmMap = new Map(existingBookmakers.map(bm => [bm.key, bm])) // Unused
