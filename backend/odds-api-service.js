...
app.post('/prematch_live_odds', async (req, res) => {
  const { sport, regions = ALL_REGIONS, markets = COMMON_MARKETS, api_key: apiKey, bookmakers } = req.body
  ...
  // Check cache first
  const cachedData = oddsCache.get(cacheKey)
  if (cachedData) {
...
app.post('/scores', async (req, res) => {
  const { sport, days_from: daysFrom = 3, api_key: apiKey } = req.body
  ...
  const cacheKey = `${sport}_${daysFrom}`

  // Check cache first
  const cachedData = scoresCache.get(cacheKey)
  if (cachedData) {
